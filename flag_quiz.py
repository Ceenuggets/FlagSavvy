...
external_stylesheets = [dbc.themes.BOOTSTRAP, "/assets/flag_quiz.css"]
...
def display_country_info(country_names):
    ...
    if not country_names:
        ...
        return report, f"{(10 - (len(random_ten))) + 1} of 10", f"/assets/flags/{random_ten[0]}.gif", options, random_ten[0], random_ten
    else:
        ...
        return report, f"{(10 - (len(country_names)) + 1)} of 10", f"/assets/flags/{country_names[0]}.gif", options, country_names[0], country_names
            ...
            for option in options:
                ...
                if label != correct_answer and label == select_answer:
                    ...
                    if len(result.values()) == 10:
                        img_results = [
                            html.Div([html.Div([
                                html.Img(src=f"/assets/flags/{country}.gif",
                                         style={'width': '80px', 'height': '60px'}),
                                html.P(country, style={'text-align': 'left', 'margin-top': '0px', 'margin-bottom': '0px', 'color': 'white'}),
                            ],className="country_flags_results"),
                        ], style={'display': 'flex', 'flex-direction': 'column', 'align-items': 'center', 'margin-top': '5px'}) for country in all_correct_answers]
